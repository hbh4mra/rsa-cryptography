	...
	public class MainForm : Form
	{
		...
		public MainForm( string fileName )
		{
			...
			if( File.Exists( fileName ) )
			{
				...
				streamReader.Close();				
				this.Text = GetFileName( fileName ) + " - RSA Crypto";
				cleanForm = true;
			}
		}
		...
		private void InitializeComponent()
		{
            ...
            this.Text = "RSA Crypto Build 6.7.16_04_13_12_18";
		...
		private string openFile( string title, string filterString )
		{
			...
			if( openFileDialog.ShowDialog() == DialogResult.OK )
			{
				if( File.Exists( openFileDialog.FileName ) )
				{
					...
					if( fileString != null )
					{
						this.Text = GetFileName( openFileDialog.FileName ) + " - RSA Crypto";
		...
		private bool saveFile()
		{
			...
			if( saveFileDialog.ShowDialog() == DialogResult.OK )
			{
				try
				{
					...
					streamWriter.Close();
					this.Text = GetFileName( saveFileDialog.FileName ) + " - RSA Crypto";
		...
		private void mainForm_Load( object sender, EventArgs e )
		{
			...
			this.Text = GetFileName( currentFileName ) + " - RSA Crypto";
			inputTextBox.Focus();
		...
		private void mainForm_FormClosing( object sender, FormClosingEventArgs e )
		{
			...
			if( !cleanForm )
			{
				...
				switch( MessageBox.Show( dialogText, "RSA Crypto", MessageBoxButtons.YesNoCancel ) )
				{
		...
		private void inputTextBox_DragDrop( object sender, DragEventArgs e )
		{
			...
			if( clean )
			{
				if( e.Data.GetDataPresent( DataFormats.FileDrop, true ) == true )
				{
					...
					this.Text = GetFileName( fileNames[ 0 ] ) + " - RSA Crypto";
